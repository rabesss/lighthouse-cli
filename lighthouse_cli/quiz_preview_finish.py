...
class PreviewSubmitUnknownError(NetworkError):
    def __init__(self) -> None:
        super().__init__(
            "Preview submission could not be verified. Inspect the result before retrying."
        )
    ...
    *,
    ...
    try:
        body, _ = client.get_raw(
            receipt_path(course_id, quiz_id, attempt_id),
            max_bytes=MAX_PAGE_BYTES,
            _replay_safe=False,
            headers={"Cache-Control": "no-cache"},
        )
        soup = BeautifulSoup(body, "html.parser")
        heading_verified = any(
            h.get_text(" ", strip=True) == "Your work has been saved and submitted"
        ...
        detail = client.get_json(
            f"/{course_id}/quizzes/{quiz_id}/attempts/{attempt_id}", _replay_safe=False
        )
        if (
            not isinstance(detail, dict)
            or type(detail.get("AttemptId")) is not int
            or detail["AttemptId"] != attempt_id
            or type(detail.get("QuizId")) is not int
            or detail["QuizId"] != quiz_id
            or (
                actor_id is not None
                and (type(detail.get("UserId")) is not int or detail["UserId"] != actor_id)
            )
        ):
...
def _rpc_result(response: Any, quiz_id: int, attempt_id: int) -> None:
    ...
    if (
        ...
        or type(reply.get("ResponseType")) is not int
        or reply["ResponseType"] != 0
        or reply.get("IsResultMin") is not False
        or reply.get("RedirectUrl") != ""
...
def submit_preview(
    ...
    *,
    ...
    try:
        ...
        response = None
        confirmation_path = (
            "/d2l/lms/quizzing/user/attempt/quiz_confirm_submit_auto.d2l?"
            + urlencode(
                {
                    "qi": quiz_id,
                    "ai": attempt_id,
        ...
        confirmation, _ = client.get_raw(
            confirmation_path, max_bytes=MAX_PAGE_BYTES, _replay_safe=False
        )
        form, confirmation_fields = hidden_form(confirmation)
        if (
            not form.select('input[name="attemptCanBeGraded"][type="checkbox"]')
            or not any(
                b.get_text(" ", strip=True) == "Submit Quiz" and not b.has_attr("disabled")
        ...
        rpc_url = client.canonical_url(
            parent_path + "file?" + urlencode({**context, "d2l_rh": "rpc", "d2l_rt": "call"})
        )
        params = {
            "param1": str(quiz_id),
            "param2": str(attempt_id),
            "param3": True,
            "param4": retain,
            "param5": integration_flag,
            "param6": False,
            "param7": "",
        }
        response = client._request(
            ...
            stream=True,
            data={
                "d2l_rf": "ProcessQuizSubmission",
                "params": json.dumps(params),
                "d2l_referrer": protection.csrf_token,
                "d2l_hitcode": protection.next_hit_code(),
                "d2l_action": "rpc",
            },
        ...
        result = verify_receipt(
            client, course_id=course_id, quiz_id=quiz_id, attempt_id=attempt_id, actor_id=actor_id
        )

...
def page(number: int = 1, saved: bool = True):
    return parse_preview_page(
        html(question(number, number, saved="True" if saved else "False"), page=number),
        course_id=10,
        quiz_id=20,
        attempt_id=30,
        page=number,
...
def remote():
    ...
    client.get_quiz_detail.return_value = {
        "PagingTypeId": 1,
        "PreventMovingBackwards": True,
        "IsSingleSession": False,
        "SubmissionTimeLimit": {"IsEnforced": False},
    }
...
def test_start_readback_uncertainty_is_durable_and_blocks_restart(remote):
    workflow = PreviewWorkflow("trial", 10, 20)
    with patch(
        "lighthouse_cli.quiz_preview_session.start_preview", side_effect=PreviewStartUnknownError()
    ):
...
def test_uncertain_save_blocks_writes_and_recovers_by_readback(remote):
    ...
    start_local(workflow)
    with patch(
        "lighthouse_cli.quiz_preview_session.save_current_preview_answer",
        side_effect=PreviewSaveUnknownError(),
    ) as save:
    ...
    assert workflow.status()["status"] == "uncertain"
    with patch(
        "lighthouse_cli.quiz_preview_session.read_current_preview", return_value=page()
    ) as read:
...
def test_uncertain_advance_stays_blocked_without_authoritative_cursor(remote):
    ...
    start_local(workflow)
    with patch(
        "lighthouse_cli.quiz_preview_session.advance_current_preview",
        side_effect=PreviewAdvanceUnknownError(),
    ) as advance:
...
def test_commit_failure_after_advance_never_restores_old_active_cursor(remote):
    ...
    with (
        patch.object(workflow.store, "write_artifact", side_effect=fail_commit),
        patch(
            "lighthouse_cli.quiz_preview_session.advance_current_preview", return_value=page(2)
        ) as advance,
    ):
...
def test_completed_remote_attempt_is_not_submitted_again(remote):
    ...
    with (
        patch("lighthouse_cli.quiz_preview_session.verify_receipt", return_value=receipt),
...
def test_cli_dry_run_and_declined_write_do_not_open_credentials():
    with patch("lighthouse_cli.quiz_preview_commands.PreviewWorkflow") as workflow:
        ...
        declined = CliRunner().invoke(
            cli, ["instructor", "--site", "trial", "preview", "start", "10", "20", "--json"]
        )
...
def test_cli_error_is_json_only_and_secret_safe():
    with patch(
        "lighthouse_cli.quiz_preview_commands.PreviewWorkflow",
        side_effect=RuntimeError("cookie=SECRET_SENTINEL"),
    ):
        result = CliRunner().invoke(
            cli, ["instructor", "--site", "trial", "preview", "page", "10", "20", "--json"]
        )

...
def test_extracts_only_script_bootstrap_not_user_content():
    assert (
        csrf_from_homepage(b"""<div>localStorage.setItem('XSRF.Token','fake')</div>
    <script>localStorage.setItem('XSRF.Token', 'synthetic-csrf');</script>""")
        == "synthetic-csrf"
    )
...
@pytest.mark.parametrize(
    "body",
    [
        b"<script>no bootstrap</script>",
        b"<script>localStorage.setItem('XSRF.Token', 'line\nbreak')</script>",
        b"<script>localStorage.setItem('XSRF.Token','a');localStorage.setItem('XSRF.Token','b')</script>",
    ],
)
...
def test_bootstrap_cached_for_same_client():
    ...
    client.get_raw = Mock(
        return_value=(b"<script>localStorage.setItem('XSRF.Token','synthetic-csrf')</script>", {})
    )
    ...
    record = json.dumps(
        {
            ...
            "P": ["d2l_referrer", "SYNTHETIC_TOKEN", 1234567890],
        }
    )

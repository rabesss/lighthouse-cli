...
def client_for_submit(*, rpc_result: str | None = None, secure_browser: str = "0"):
    ...
    client.get_json = Mock(
        return_value={
            ...
            "QuizId": 20,
            "UserId": 7,
            "Completed": "2026-09-17T15:00:00Z",
            "Score": 1,
        }
    )
    reply = {
        "ResponseType": 0,
        "IsResultMin": False,
        "Result": rpc_result or "parent.QuizDone(20,30,'1','0', '0', 'gotoSv', '')",
        "RedirectUrl": "",
        "MessageArea": {},
    }
...
def test_submit_is_preview_only_and_verifies_independent_receipt():
    ...
    result = submit_preview(
        client, course_id=10, quiz_id=20, attempt_id=30, page=1, retain=True, actor_id=7
    )
...
@pytest.mark.parametrize(
    "result",
    [
        "parent.QuizDone(20,999,'1','0','0','gotoSv','')",
        "parent.QuizDone(20,30,'0','0','0','gotoSv','')",
        "parent.QuizDone(20,30,'1','0','0','gotoSv','');evil()",
    ],
)
...
def test_success_heading_without_completed_attempt_record_is_not_a_receipt():
    ...
    client.get_json = Mock(
        return_value={"AttemptId": 30, "QuizId": 20, "UserId": 7, "Completed": None}
    )
    with pytest.raises(PreviewSubmitUnknownError):
...
def test_submit_receipt_auth_expiry_is_unknown_after_write_dispatch():
    ...
    client.get_raw = Mock(
        side_effect=[calls[0], calls[1], calls[2], SessionExpiredError("session expired")]
    )
    with pytest.raises(PreviewSubmitUnknownError):
        submit_preview(client, course_id=10, quiz_id=20, attempt_id=30, page=1)
